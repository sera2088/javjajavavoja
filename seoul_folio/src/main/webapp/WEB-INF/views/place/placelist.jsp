		...
		<ul class="place_page">
			...
			<c:if test="${pv.startPage>1}">
				<li class="page-item"><a class="page-link"
					href="${path}/place/placelist.do?currentPage=${pv.startPage-pv.blockPage}">Prev</a></li>
			</c:if>
			<!-- 이전 출력 끝 -->
				...
				<li><c:choose>
						<c:when test="${i==pv.currentPage}">
							<a class="page-link page-item active"
								href="${path}/place/placelist.do?currentPage=${i}">${i}</a>
						</c:when>
						<c:otherwise>
							<a class="page-link" href="${path}/place/placelist.do?currentPage=${i}">${i}</a>
						</c:otherwise>
					</c:choose></li>
			</c:forEach>
			<!-- 페이지 출력 끝 -->
			...
			<c:if test="${pv.endPage<pv.totalPage}">
				<li><a class="page-link"
					href="${path}/place/placelist.do?currentPage=${pv.startPage+pv.blockPage}">Next</a></li>
			</c:if>
			<!-- 다음 출력 끝 -->
		</ul>
	</div>
